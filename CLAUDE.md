...
- 新しい外部ライブラリを追加しない。
- 親子によるデザイン作業では、ターミナルやコマンドを使用しない。
- `npm install`、`npm run`、`npx`、テスト、lint、ビルド、開発サーバーの起動を実行しない。
- Git操作を実行しない。
- 新しいファイルやフォルダを作成しない。
- 依頼されたグループの`index.html`と`style.css`以外は編集しない。
...
親子によるデザイン作業の確認は、保存後に`index.html`をブラウザーで直接開くか更新して人が行います。Claude Codeは自動確認のためのコマンドを実行せず、2ファイルの編集と変更内容の説明だけを行ってください。
