...
アプリ内読取はHTTPS環境で動作し、iPhoneのSafari／ホーム画面へ追加したPWAとAndroidのChromeに対応します。ブラウザー標準のQR検出機能がないiPhoneでは、`qr-scanner`の端末内処理へ自動的に切り替えます。映像や読み取った内容を外部APIへ送信することはありません。
