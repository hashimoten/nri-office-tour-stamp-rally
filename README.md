...
Gitは通常、リポジトリ内の1フォルダだけを単独でcloneできません。また、各チームの`index.html`は親階層にある共通CSS・JavaScript・PWAファイルを読み込むため、チームフォルダだけを切り離すとアプリの動作確認ができません。

スタッフはリポジトリ全体をcloneした状態で開発サーバーを起動し、親子のClaude Codeには担当チームのフォルダだけを作業フォルダとして開いてください。親子が編集するのは、その中の`index.html`と`style.css`だけです。
