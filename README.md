...
チェックポイントは`shared/checkpoints.js`だけで管理します。この配列へ項目を追加したり削除したりすると、全23グループのスタンプカード、合計数、進捗率、QRコード判定へ自動的に反映されます。各グループのHTMLを編集する必要はありません。
...
たとえば`id`が`studio`なら、QRコードのURLは`?point=studio`です。変更後は`npm run check`を実行し、新しいURLでスタンプを取得できることを確認してください。なお、チームフォルダの`index.html`を直接開く画像埋め込み専用表示では、共通JavaScriptを実行しないため初期の5枚が表示されます。実際のスタンプラリー画面は開発サーバーまたは公開URLで確認してください。
