...
各グループの`index.html`には、2個取得済み・3個未取得の「デザイン確認用見本」が最初から書かれています。そのため、ファイルをダブルクリックして直接開いても、進捗バー、5枚のカード、空欄の台紙、押されたスタンプを確認できます。

ViteやGitHub Pagesからアプリを起動した場合も、同じHTMLとCSSを使用します。共通JavaScriptはカードを作り直さず、見本に書かれた5枚のカードへlocalStorageの実際の取得状況だけを反映します。見本の取得日時や状態は保存データではありません。
